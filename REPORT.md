...
## 9.5b 机制：为什么 GRPO 抬高 greedy 却压低 best-of-N/自评（多样性坍缩 + 例子）
GRPO 最大化期望奖励 = 把概率质量**堆到自己最好的那条模式上**（KL/熵系数都很小，拦不住）。于是贪心 argmax 那条被强化
（**greedy↑**），但 T=1 采样越来越"自我重复"、**不同的正确候选变少**，best-of-N/自评能挑的就少（**bv↓**）。这是
**熵/多样性坍缩**。我们用诊断脚本 `scripts/eval_diversity.py`（每题采 16 个，统计去重）直接量化：

| 模型 | 唯一式子数/16 | **唯一"正确"式子数** | 坍缩率(≤2唯一) | bv@16 |
|---|---|---|---|---|
| SFT-multi（仅SFT） | 11.9 | **0.73** | 0% | 52% |
| SFT-single（仅SFT） | 11.0 | 0.61 | 0% | 46% |
| A4a（SFT→GRPO） | 9.8 | 0.56 | 6% | 46% |
| A3（shaped GRPO） | 6.4 | 0.57 | 9% | 37% |
| **R1（只GRPO无SFT）** | **15.5** | **0.06** | 0% | 6% |

- **GRPO 使唯一式子数 11.9 → 6.4–9.8、唯一正确式子 0.73 → 0.56**（A3 shaped 坍得最狠 6.4 → bv 最低）——**这正是 best-of-N/自评下降的直接原因**。
- **R1（只 GRPO、无 SFT）= "多样但不会做"**：唯一式子 15.5（最高）但唯一正确仅 **0.06**、bv 6% ≈ base。

**例子**（puzzle `[1,2,4,7]`，正确解如 `(1-2+7)*4=24`；`×n` 为该式在 16 个采样中出现次数）：
```
SFT-multi(仅SFT) : 唯一10, 含正确1 → (1-2+7)*4 ✓ ; 主模式 (7-1*2)*4 出现 5 次
A4a(SFT→GRPO)    : 唯一7(↓), 含正确1 → 主模式 (7-1*2)*4 升到 6 次, (1-2+7)*4 ✓ 2 次   ← 更尖、更集中
R1(只GRPO无SFT)  : 唯一15 但 0 正确 → (1+2)*8(照抄 prompt 示例、无视题目数字!)、(2+4)*7=24(=42 错)、"=36"、残式
```
puzzle `[4,5,6,10]` 的 R1 进一步出现 `6*(4-0.5)=24`、`-6*11=24`、`4+6+5-1=24`（用了不存在的数字）等**瞎编**。
**结论**：**SFT 提供"会做"（唯一正确式子非零），GRPO 只负责"把会做的尖锐化"（greedy↑、多样性↓）；没有 SFT，
纯 GRPO 在稀疏奖励下学不动**（零梯度组 + 照抄示例）。这解释了 §9.3/§9.5 的全部走向。
